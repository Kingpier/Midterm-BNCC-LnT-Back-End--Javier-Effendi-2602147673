...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
        ...
        // Retrieve form data
        $userId = $_POST['userId']; // Add a hidden input field for userId in your form
        $photo = $_POST['photo'];
        ...
        if (empty($errors)) {
            $result = updateUser($userId, $photo, $firstName, $lastName, $email, $bio);

            if ($result) {
                header('Location: dashboard.php');
                exit();
            } else {
                $error = "Error updating the user. Please try again.";
            }
        }
    }
}
